...
tint=[0:0.01:300];
Sint=deval(sol,tint);
eta11=phi(1)*Sint(1,:)+phi(2)*Sint(3,:)+phi(3)*Sint(5,:)+phi(4)*Sint(7,:)+phi(5)*Sint(9,:);
eta12=phi(1)*Sint(2,:)+phi(2)*Sint(4,:)+phi(3)*Sint(6,:)+phi(4)*Sint(8,:)+phi(5)*Sint(10,:);
IHB2=[tint(end-1000:end)' eta11(end-1000:end)'];

figure(1)
plot(eta,eta2,'b','linewidth',1)
hold on
plot(eta11(end-1000:end),eta12(end-1000:end),'-.r','linewidth',1);
grid on
%title('范德波极限环')
legend('IHB-5','dde23');
xlabel('x0')
...
s=xlswrite('C:\Users\Administrator\Desktop\IHB\comparison\IHB_1992_5dof\order_1\IHB_3P6_order1.xls',result);
toc